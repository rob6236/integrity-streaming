...
const NAV = [
  ...
  { label: "Thumbnail Designer", href: "/creator-studio/thumbnail" },
...
export default function CreatorStudioLayout({
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (u) => {
      setUser(u);
      if (!u) {
        router.replace(`/login?next=${encodeURIComponent("/creator-studio")}`);
      }
    });
    return () => unsub();
  }, [router]);
    ...
    >
        ...
        >
          ...
          <h1
            style={{
              ...
              letterSpacing: 0.4,
              textShadow:
                "0 1px 0 rgba(0,0,0,0.45), 0 0 10px rgba(255,215,0,0.28)",
              color: GOLD,
              fontSize: 28,
              textAlign: "center",
            }}
      ...
      <style jsx global>{`
        ...
        .studio-smiley {
          ...
          height: 34px;
          border-radius: 50%;
          background: radial-gradient(circle at 30% 30%, #ffd27a, #ffb84d);
          border: 2px solid ${GOLD};
          box-shadow: 0 0 0 1px rgba(255,215,0,0.5),
                      inset 0 0 10px rgba(0,0,0,0.15);
          animation: dance-bounce 1.8s infinite ease-in-out;
        }
        .studio-smiley .eye {
          position: absolute;
          top: 10px;
        ...
        .studio-smiley .eye.left { left: 9px; }
        .studio-smiley .eye.right { right: 9px; }
        ...
        .studio-smiley .arm.right {
          height: 14px;
          right: -5px;
          top: 12px;
          transform: rotate(-30deg);
          transform-origin: bottom;
          animation: wave 1.8s infinite ease-in-out reverse;
        }
        .studio-smiley .leg.left {
          height: 16px;
          left: 8px;
          bottom: -12px;
          transform: rotate(10deg);
          transform-origin: top;
          animation: step 1.8s infinite ease-in-out;
        }
        .studio-smiley .leg.right {
          height: 16px;
          right: 8px;
          bottom: -12px;
          transform: rotate(-10deg);
          transform-origin: top;
          animation: step 1.8s infinite ease-in-out reverse;
        }
...
function Smiley({ position }: { position: "left" | "right" }) {
  ...
  return (
    <div
      ...
      style={{
        left: position === "left" ? inset : undefined,
        right: position === "right" ? inset : undefined,
      }}
      aria-hidden
    >
